...
import dhg.ccg.parse.scg.mcmc.SimpleScgProductionCounterI
import dhg.ccg.parse.scg.mcmc.TrainDataScgAlphaPriorMakerI

class PcfgAlphaPriorMakerITests {
  ...
  @Test
  def i_test_TrainDataPcfgAlphaPriorMaker {
    ...
    val alphaPriorMakerI = new TrainDataPcfgAlphaPriorMakerI(catIndexer, wordIndexer)
    val (alphaPriorRootCounts, alphaPriorBinyCounts, alphaPriorUnryCounts, alphaPriorTermCounts, alphaPriorPmixCounts) =
      alphaPriorMakerI.makeAll(
        ...
        alphaRoot = 0.11, alphaBiny = 0.12, alphaUnry = 0.13, alphaTerm = 0.14, alphaPmix = 0.15,
        numCats: Int, numWords: Int): ( //
