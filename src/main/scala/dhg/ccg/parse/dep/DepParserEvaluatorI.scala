...
class DepParserEvaluatorI(printEverything: Boolean = false)( //
    catIndexer: Indexer[Cat], wordIndexer: Indexer[String]) { //extends ParserEvaluator {
  type Word = String

  def evaluate(model: PcfgParserI, testData: Iterable[(Option[CfgGuideChartI], CcgTreeI)], //tagdict: TagDictionary[Cat],
    ...
    writeUsing(File(outputFile.getOrElse("/dev/null"))) { output =>
      time("testing", {
        for (((guideChart, goldTreeI), i) <- testData.iterator.zipWithIndex) {
          val goldTree = CcgTreeI.from(goldTreeI, catIndexer, wordIndexer)
          ...
          val modelParsed = guideChart.map(gc => CcgTreeI.from(model.parseAndLogProbFromGuideChart(gc)._1, catIndexer, wordIndexer))
  ...
  override def toString = f"DepParserEvaluator()"
