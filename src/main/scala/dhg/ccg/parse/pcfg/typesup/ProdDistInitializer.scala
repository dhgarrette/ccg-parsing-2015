...
trait ProdDistInitializer {
  def apply(sentences: Vector[CfgGuideChart], initialTagdict: TagDictionary[Cat]): ConditionalLogProbabilityDistribution[Cat, Prod]
}

class UniformProdDistInitializer() extends ProdDistInitializer {
  def apply(sentences: Vector[CfgGuideChart], initialTagdict: TagDictionary[Cat]): ConditionalLogProbabilityDistribution[Cat, Prod] = {
  ...
  override def toString = f"UniformProdDistInitializer()"
}
